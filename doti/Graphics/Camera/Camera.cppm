...
private:
    ...
    static auto registerEvents(Camera* camera) -> void {
        ...
        if (!registered) {
            ...
            EventManager::connect<>("Camera::MoveLeft", [camera]() {
                camera->_cameraPos -= camera->_cameraRight * camera->_cameraSpeed;
            });
            EventManager::connect<>("Camera::MoveRight", [camera]() {
                camera->_cameraPos += camera->_cameraRight * camera->_cameraSpeed;
            });
            EventManager::connect<>("Camera::MoveForward", [camera]() {
                camera->_cameraPos += camera->_cameraFront * camera->_cameraSpeed;
            });
            EventManager::connect<>("Camera::MoveBackward", [camera]() {
                camera->_cameraPos -= camera->_cameraFront * camera->_cameraSpeed;
            });
            registered = true;
        }
